cv.sxenv <- function (X, Y, u, R, m, nperm) {
  Y <- as.matrix(Y)
  a <- dim(Y)
  n <- a[1]
  r <- a[2]
  ...
  for (i in 1:nperm) {
    ...
    for (j in 1:m) {
      ...
      fit <- sxenv(X.train, Y.train, u, R, asy = F)
      betahat <- fit$beta
      muYhat <- fit$muY
      muXhat <- fit$muX
      muhat <- muYhat - crossprod(betahat, muXhat)
      resi <- as.matrix(Y.test - matrix(1, n.test, 1) %*% 
                          t(muhat) - as.matrix(X.test) %*% betahat)
