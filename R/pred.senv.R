pred.senv <- function (m, Xnew) 
{
  if (is.null(m$ratio)) 
    stop("The asymptotic distribution part of senv is missing. Rerun senv with asy = T")
  r <- ncol(m$Sigma)
  n <- m$n
  if (is.null(m$Gamma)) {
    u <- 0
  ...
  if (u == 0) {
    ...
    if (nrow(Xnew) == 1) 
      Xnew <- t(Xnew)
    value <- m$mu + m$beta %*% Xnew
    temp <- kronecker(Xnew, diag(r))
    covMatrix.estm <- m$Sigma/n + crossprod(temp, m$covMatrix) %*% 
      temp/n
    covMatrix.pred <- (1 + 1/n) * m$Sigma + crossprod(temp, 
                                                      m$covMatrix) %*% temp/n
  }
